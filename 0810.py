...
X = df_result[['X1', 'X2']].copy()

# 2차 항 추가
X['X1_sq'] = X['X1'] ** 2  # X1^2 항 추가
X['X2_sq'] = X['X2'] ** 2  # X2^2 항 추가
...
# statsmodels로 분석
# 1. 수정된 독립 변수 X를 사용
X_poly = df_result[['X1', 'X2']]
X_poly['X1_sq'] = X_poly['X1'] ** 2
X_poly['X2_sq'] = X_poly['X2'] ** 2

# 2. 절편항 추가
X_poly = sm.add_constant(X_poly)

# 3. statsmodels.api를 사용한 회귀 분석
sm_model_Y1 = sm.OLS(df_result['efficiency1'], X_poly).fit()
...
y1_grid = sm_model_Y1.params['const'] + \
          sm_model_Y1.params['X1'] * x1_grid + \
          sm_model_Y1.params['X2'] * x2_grid + \
          sm_model_Y1.params['X1_sq'] * (x1_grid**2) + \
          sm_model_Y1.params['X2_sq'] * (x2_grid**2)
...
y2_grid = sm_model_Y2.params['const'] + \
          sm_model_Y2.params['X1'] * x1_grid + \
          sm_model_Y2.params['X2'] * x2_grid + \
          sm_model_Y2.params['X1_sq'] * (x1_grid**2) + \
          sm_model_Y2.params['X2_sq'] * (x2_grid**2)
ax2.plot_surface(x1_grid, x2_grid, y2_grid, alpha=0.5, cmap='plasma')
ax2.set_xlabel('X1 (충전 평균 온도)')
ax2.set_ylabel('X2 (주행 평균 온도)')
ax2.set_zlabel('효율 Y2')
