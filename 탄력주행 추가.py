...
for (vehicle, month), df in bms_df.groupby(['vehicle', 'year_month']):
    ...
    if X4 > 0:
        X3_new = X3 / (X4 / 1000)  # 주행거리를 km 단위로 변환하여 계산
    else:
        X3_new = np.nan
    ...
    # 새로운 독립변수 X4_new (한 달 총 회생제동 에너지)
    E_regen = calc_energy(df[voltage_col], -df[current_col], df[time_interval])[cond_regen].sum()
    X4_new = E_regen

    # 새로운 독립변수 X5_new (한 달 주행 중 평균 속도)
    cond_driving_only = (df[speed_col] > 0) & (df[cable_col] == 0) & (df[current_col] > 0)
    X5_new = df.loc[cond_driving_only, speed_col].mean()

    # 새로운 독립변수 X6_new (한 달 총 탄력주행 거리)
    cond_coasting = (df[speed_col] > 0) & (df[cable_col] == 0) & (abs(df[current_col]) < 1)
    coasting_df = df.loc[cond_coasting].copy()
    if 'odometer' in coasting_df.columns:
        coasting_df['delta_distance'] = coasting_df['odometer'].diff().fillna(0)
        X6_new = coasting_df['delta_distance'].sum()
    else:
        X6_new = (coasting_df['speed'] / 3.6 * coasting_df['delta_sec']).sum()
...
# X3_new, X4_new, X5_new, X6_new는 1차항으로만 추가하므로 중심화하지 않음
X_poly['X3_new'] = df_result['X3_new']
X_poly['X4_new'] = df_result['X4_new']
X_poly['X5_new'] = df_result['X5_new']
X_poly['X6_new'] = df_result['X6_new']
...
x4_new_mean = df_result['X4_new'].mean()
x5_new_mean = df_result['X5_new'].mean()
x6_new_mean = df_result['X6_new'].mean()
...
y1_grid = sm_model_Y1.params['const'] + \
          ...
          sm_model_Y1.params['X4_new'] * x4_new_mean + \
          sm_model_Y1.params['X5_new'] * x5_new_mean + \
          sm_model_Y1.params['X6_new'] * x6_new_mean
...
y2_grid = sm_model_Y2.params['const'] + \
          ...
          sm_model_Y2.params['X4_new'] * x4_new_mean + \
          sm_model_Y2.params['X5_new'] * x5_new_mean + \
          sm_model_Y2.params['X6_new'] * x6_new_mean
