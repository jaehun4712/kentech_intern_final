...
# 상관계수 계산
# -----------------------
r1, p1 = pearsonr(weekly_df['ext_temp_avg'], weekly_df['efficiency1'])
r2, p2 = pearsonr(weekly_df['ext_temp_avg'], weekly_df['efficiency2'])

# -----------------------
# 시각화
# -----------------------
fig, axs = plt.subplots(2, 1, figsize=(10, 12))
sns.scatterplot(data=weekly_df, x='ext_temp_avg', y='efficiency1', hue='vehicle', ax=axs[0])
sns.regplot(data=weekly_df, x='ext_temp_avg', y='efficiency1', scatter=False, ax=axs[0], color='gray')
axs[0].set_title(f'온도 vs 효율 e1')

sns.scatterplot(data=weekly_df, x='ext_temp_avg', y='efficiency2', hue='vehicle', ax=axs[1])
sns.regplot(data=weekly_df, x='ext_temp_avg', y='efficiency2', scatter=False, ax=axs[1], color='gray')
...
plt.xlabel('외기온도(°C)')
plt.ylabel('효율 e1 (%)')
plt.title('외기온도 vs 효율 e1 (전체 차량 데이터)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.xlabel('외기온도(°C)')
plt.ylabel('효율 e2 (%)')
plt.title('외기온도 vs 효율 e2 (전체 차량 데이터)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
