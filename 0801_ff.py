...
# 1) pack_volt 문자열에서 쉼표 기준 숫자 분리 후 평균 구하기
def mean_pack_volt(row):
    try:
        volt_list = [float(v.strip()) for v in row.split(',') if v.strip() != '']
        return np.mean(volt_list) if volt_list else np.nan
    except:
        return np.nan

bms_df['avg_pack_volt'] = bms_df['pack_volt'].astype(str).apply(mean_pack_volt)

# 2) OCV -> SOC 역보간 함수 생성 (ocv_vals 기준 오름차순 정렬 후)
# 이미 soc_vals, ocv_vals 정렬되어 있음
# OCV 기준으로 SOC를 보간하는 함수 (역함수 역할)
ocv_to_soc_func = interp1d(ocv_vals, soc_vals, kind='linear', fill_value='extrapolate')

# 3) bms_df에 avg_pack_volt 기반으로 soc_new 계산
bms_df['soc_new'] = ocv_to_soc_func(bms_df['avg_pack_volt'])
