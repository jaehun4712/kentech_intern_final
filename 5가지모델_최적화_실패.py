...
for path, vehicle_name in file_info.items():
    ...
    for month, df in bms_df.groupby('year_month'):
        ...
        total_driving_distance_m = df.loc[
            cond_drive_section, 'delta_dist'].sum() if 'delta_dist' in df.columns else np.nan
        total_driving_time_sec = df.loc[cond_drive_section, 'delta_sec'].sum()
        if total_driving_time_sec > 0 and total_driving_distance_m > 0:
            X5_new = (total_driving_distance_m / 1000) / (total_driving_time_sec / 3600)
        else:
            X5_new = np.nan
...
independent_vars = ['X1', 'X2', 'X3_new', 'X5_new', 'X_eff_time_new']
titles = {
    'X1': 'X1 (충전 평균 온도) - °C',
    'X2': 'X2 (주행 평균 온도) - °C',
    'X3_new': 'X3 (1km당 급가속 횟수) - 횟수/km',
    'X5_new': 'X5 (평균 속도) - km/h',
