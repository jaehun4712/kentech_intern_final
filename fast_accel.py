...
plt.scatter(merged['n_accel'], merged['e_trip_eff'], label='급가속 횟수', alpha=0.6)
plt.scatter(merged['n_decel'], merged['e_trip_eff'], label='급감속 횟수', alpha=0.6)
plt.xlabel("이벤트 수")
plt.ylabel("e_trip 효율 (%)")
plt.title("급가속/급감속 이벤트 수와 효율 관계")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
