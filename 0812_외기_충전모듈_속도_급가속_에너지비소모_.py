...
try:
    ...
    print("오류: OCV 파일을 찾을 수 없습니다. 경로를 확인해주세요.")
    # 파일이 없는 경우, 더 이상 진행할 수 없으므로 종료합니다.
    exit()
...
# 파일별로 데이터를 읽고 처리하는 루프
for path, vehicle_name in file_info.items():
    print(f"처리 중인 파일: {path}")
    try:
        bms_df = pd.read_csv(path)
    except FileNotFoundError:
        print(f"오류: {path} 파일을 찾을 수 없어 건너뜁니다.")
        continue

    bms_df['vehicle'] = vehicle_name
    ...
    for month, df in bms_df.groupby('year_month'):
        ...
        X1 = df.loc[cond_drive_section,'mod_temp_avg'].mean() #주행중 평균 모듈온도

        #X2 = df.loc[cond_drive_section, 'mod_temp_avg'].mean()
        X2 = df.loc[cond_drive_section, 'soc'].mean()
        ...
        X_drive_speed = df.loc[cond_drive_section, 'speed'].mean()
        ...
        if len(soc_series) < 2:
            efficiency1, efficiency2 = np.nan, np.nan
        else:
        ...
        monthly_result.append({
            ...
            'X1': X1, #주행 구간 평균 외기온도, 이차항포함
            'X2': X2, #주행 구간 평균 soc
            'X3_new': X3_new, #1km당 급가속횟수
            'X_eff_time_new': X_eff_time_new, # 에너지 비 소모 시간(회생제동+탄력주행)
            'X_drive_speed': X_drive_speed # 주행구간 평균속도
        })

# -----------------------
# 회귀분석 및 시각화 (기존 코드와 동일)
# -----------------------
df_result = pd.DataFrame(monthly_result).dropna()

if df_result.empty:
    print("오류: 분석할 유효한 데이터가 없어 회귀분석 및 시각화를 건너뜁니다.")
    exit()
...
X_poly['X1_X2_interaction'] = X_poly['X1_centered'] * X_poly['X2_centered']

X_poly['X3_new'] = df_result['X3_new']
X_poly['X_eff_time_new'] = df_result['X_eff_time_new']
X_poly['X_drive_speed'] = df_result['X_drive_speed']
...
y1_grid = sm_model_Y1.params['const'] + \
          ...
          sm_model_Y1.params['X_drive_speed'] * x_drive_speed_mean
...
y2_grid = sm_model_Y2.params['const'] + \
          ...
          sm_model_Y2.params['X_drive_speed'] * x_drive_seppd_mean
...
# 3. 각 독립변수와 효율의 관계 시각화
independent_vars = ['X1', 'X2', 'X3_new', 'X_eff_time_new', 'X_drive_speed']
titles = {
    'X1': 'X1 (충전 평균 온도) - °C',
    'X2': 'X2 (주행 평균 온도) - °C',
    'X3_new': 'X3 (1km당 급가속 횟수) - 횟수/km',
    'X_eff_time_new': 'X (총 에너지 비소모 주행 시간) - 초',
     'X_drive_speed': 'X (평균속도) - km/s'
}
...
for i, var in enumerate(independent_vars):
    ...
    if var in ['X1', 'X2']:
        ...
        y_pred = sm_model_Y1.predict(pred_df[['const', 'X1_centered', 'X1_sq_centered','X2_centered', 'X2_sq_centered',
                                              'X3_new', 'X_eff_time_new', 'X_drive_speed']])
...
for i, var in enumerate(independent_vars):
    ...
    if var in ['X1', 'X2']:
        ...
        pred_df = pd.DataFrame(fixed_values, index=x_values)
        pred_df['const'] = 1
        y_pred = sm_model_Y2.predict(pred_df[['const', 'X1_centered', 'X2_sq_centered','X2_centered', 'X2_sq_centered',
                                              'X3_new', 'X_eff_time_new', 'X_drive_speed']])
        sns.lineplot(x=x_values, y=y_pred, ax=axes[i], color='green', linewidth=2, label='예측 회귀선')
...
x3_new_mean = df_result['X3_new'].mean()
x_eff_time_new_mean = df_result['X_eff_time_new'].mean()
x_drive_speed_mean = df_result['X_drive_speed'].mean()
...
ax1.plot_surface(x1_grid, x2_grid, y1_grid, alpha=0.5, cmap='viridis')
ax1.set_xlabel('X1 (주행 평균 외기온도)')
ax1.set_ylabel('X2 (주행 평균 SOC)')
ax1.set_zlabel('효율 Y1')
ax1.set_title('Y1 다항회귀 결과 (새 변수는 평균값 사용)')
...
y2_grid = sm_model_Y2.params['const'] + \
          ...
          sm_model_Y2.params['X3_new'] * x3_new_mean + \
          sm_model_Y2.params['X_eff_time_new'] * x_eff_time_new_mean + \
          sm_model_Y2.params['X_drive_speed'] * x_drive_speed_mean

ax2.plot_surface(x1_grid, x2_grid, y2_grid, alpha=0.5, cmap='plasma')
ax2.set_xlabel('X1 (주행 평균 외기온도)')
ax2.set_ylabel('X2 (주행 평균 SOC)')
ax2.set_zlabel('효율 Y2')
ax2.set_title('Y2 다항회귀 결과 (새 변수는 평균값 사용)')
...
sns.scatterplot(x=y1, y=df_result['Y1_pred'], ax=axes[0, 0])
axes[0, 0].set_xlabel('실제 Y1')
axes[0, 0].set_ylabel('예측 Y1')
axes[0, 0].set_title('Y1 실제 vs 예측')
...
sns.scatterplot(x=y2, y=df_result['Y2_pred'], ax=axes[1, 0], color='red')
axes[1, 0].set_xlabel('실제 Y2')
axes[1, 0].set_ylabel('예측 Y2')
axes[1, 0].set_title('Y2 실제 vs 예측')
...
titles = {
    'X1': 'X1 (주행 평균 외기온도) - °C',
    'X2': 'X2 (주행 평균 SOC) - %',
    'X3_new': 'X3 (1km당 급가속 횟수)',
    'X_eff_time_new': 'X (에너지 비소모 시간) - 초',
    'X_drive_speed': 'X (평균속도) - km/s'
}
...
for i, var in enumerate(independent_vars):
    ...
    # X1(외기온도)에 대해서만 이차항을 포함한 회귀선 그리기
    if var == 'X1':
        x_values = np.linspace(df_result[var].min(), df_result[var].max(), 100)
        fixed_values = X_poly.mean().to_dict()
        fixed_values['X1_centered'] = x_values - X1_mean
        fixed_values['X1_sq_centered'] = (x_values - X1_mean) ** 2
        pred_df = pd.DataFrame(fixed_values, index=x_values)
        pred_df['const'] = 1

        y_pred = sm_model_Y1.predict(pred_df)
        sns.lineplot(x=x_values, y=y_pred, ax=axes[i], color='green', linewidth=2, label='예측 회귀선')
    # 나머지 변수들은 선형 회귀선 그리기
    else:
        sns.regplot(x=df_result[var], y=df_result['efficiency1'], ax=axes[i], scatter=False, color='red',
...
for i, var in enumerate(independent_vars):
    ...
    if var == 'X1':
        ...
        pred_df = pd.DataFrame(fixed_values, index=x_values)
        pred_df['const'] = 1
        y_pred = sm_model_Y2.predict(pred_df)
        sns.lineplot(x=x_values, y=y_pred, ax=axes[i], color='green', linewidth=2, label='예측 회귀선')
    # 나머지 변수들은 선형 회귀선 그리기
    else:
        sns.regplot(x=df_result[var], y=df_result['efficiency2'], ax=axes[i], scatter=False, color='red',
...
def plot_var_influence(df_result, sm_model_Y1, sm_model_Y2):
    # 독립변수명과 단위
    independent_vars = {
        'X1': '주행  평균 모듈 온도 (°C)',
        'X2': '주행 평균 soc (%)',
        'X3_new': '1km당 급가속 횟수 (회/km)',
        'X_eff_time_new': '총 에너지 비소모 주행 시간 (초)',
        'X_drive_speed': '평균속도 (km/h)'
    }

    # 1. Y1 변수별 산점도 + 2차 회귀선
    fig, axes = plt.subplots(1, len(independent_vars), figsize=(5*len(independent_vars), 4))
    fig.suptitle("Y1 변수별 산점도 + 회귀선", fontsize=16, y=1.05)
    for i, (var, label) in enumerate(independent_vars.items()):
        sns.scatterplot(x=df_result[var], y=df_result['efficiency1'], ax=axes[i], color='blue', alpha=0.6)
        sns.regplot(x=df_result[var], y=df_result['efficiency1'], ax=axes[i], scatter=False,
                    order=2, color='red', line_kws={'linestyle': '--'})
    ...
    # 2. Y2 변수별 산점도 + 2차 회귀선
    fig, axes = plt.subplots(1, len(independent_vars), figsize=(5*len(independent_vars), 4))
    fig.suptitle("Y2 변수별 산점도 + 회귀선", fontsize=16, y=1.05)
    for i, (var, label) in enumerate(independent_vars.items()):
        sns.scatterplot(x=df_result[var], y=df_result['efficiency2'], ax=axes[i], color='green', alpha=0.6)
        sns.regplot(x=df_result[var], y=df_result['efficiency2'], ax=axes[i], scatter=False,
                    order=2, color='orange', line_kws={'linestyle': '--'})
        axes[i].set_xlabel(label)
        axes[i].set_ylabel("효율 Y2")
        axes[i].grid(True)
    plt.tight_layout()
    ...
    # 3. Partial Regression Plots (순수 변수 효과)
    fig = sm.graphics.plot_partregress_grid(sm_model_Y1, fig=plt.figure(figsize=(12,8)))
    plt.suptitle("Y1 Partial Regression Plots", fontsize=16, y=1.02)
    plt.show()

    fig = sm.graphics.plot_partregress_grid(sm_model_Y2, fig=plt.figure(figsize=(12,8)))
    plt.suptitle("Y2 Partial Regression Plots", fontsize=16, y=1.02)
    plt.show()


# 실행 예시
plot_var_influence(df_result, sm_model_Y1, sm_model_Y2)
