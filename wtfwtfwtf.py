...
# 공회전 조건 정의
df['is_idling'] = (df['speed'] == 0) & (df['pack_current'] < 0) & (df['chrg_cable_conn'] == 0)

# trip 단위 정차시간 계산
idling_time_per_trip = df[df['is_idling']].groupby('trip_id')['delta_sec'].sum().reset_index()
idling_time_per_trip.columns = ['trip_id', 'idling_time']
...
# 2. 정차시간 병합 및 stop_ratio 계산
filtered_merged = filtered_merged.merge(idling_time_per_trip, on='trip_id', how='left')
filtered_merged['idling_time'] = filtered_merged['idling_time'].fillna(0)
filtered_merged['stop_ratio'] = filtered_merged['idling_time'] / filtered_merged['total_time']


# 🔽 stop_ratio, accel_ratio, avg_batt_temp 등이 모두 포함된 상태라고 가정

from sklearn.linear_model import LinearRegression
from sklearn.metrics import r2_score
import statsmodels.api as sm

features = ['accel_ratio', 'decel_ratio', 'stop_ratio']
X = filtered_merged[features]
y = filtered_merged['e_trip_eff']
...
# 효율 시각화와 함께 보여주고 싶다면, 회귀 이전 또는 이후에 삽입

plt.figure(figsize=(8, 5))
plt.scatter(filtered_merged['stop_ratio'], filtered_merged['e_trip_eff'], alpha=0.6)
plt.xlabel("정차시간 비율 (stop_ratio)")
plt.ylabel("e_trip 효율 (%)")
plt.title("정차시간 비율과 주행 효율 관계")
plt.grid(True)
plt.tight_layout()
plt.show()
