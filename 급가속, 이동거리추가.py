...
r"E:\EV6_1year\84\bms_merged_51.csv": "veh84"
...
# X3와 X4는 1차항으로만 추가하므로 중심화하지 않음
X_poly['X3'] = df_result['X3']
X_poly['X4'] = df_result['X4']
...
# Y1 회귀면 계산 (다항식)
# X3와 X4의 평균값을 사용하여 3D 평면을 그림
x3_mean = df_result['X3'].mean()
x4_mean = df_result['X4'].mean()
...
y1_grid = sm_model_Y1.params['const'] + \
          ...
          sm_model_Y1.params['X3'] * x3_mean + \
          sm_model_Y1.params['X4'] * x4_mean
...
ax1.set_title('Y1 다항회귀 결과 (X3, X4는 평균값 사용)')
...
y2_grid = sm_model_Y2.params['const'] + \
          ...
          sm_model_Y2.params['X3'] * x3_mean + \
          sm_model_Y2.params['X4'] * x4_mean
...
ax2.set_title('Y2 다항회귀 결과 (X3, X4는 평균값 사용)')
