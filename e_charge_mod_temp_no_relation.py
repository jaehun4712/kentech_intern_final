...
for month, month_df in bms_df.groupby('year_month'):
    ...
    monthly_result.append(
        {'month': month, 'efficiency1': e1, 'efficiency2': e2, 'e_charge': e_chg_ratio, 'mod_temp_avg': avg_temp})
...
# 상관계수
corr_matrix = monthly_df[['efficiency1', 'efficiency2', 'e_charge', 'mod_temp_avg']].corr()
print("\n📊 상관계수 행렬:")
print(corr_matrix)

# 시각화
fig, axs = plt.subplots(3, 1, figsize=(14, 12), sharex=True)
sns.lineplot(data=monthly_df, x='month', y='mod_temp_avg', ax=axs[0], color='tab:blue')
axs[0].set_title('월별 평균 모듈온도')
...
# 상관계수 및 유의확률 계산
r1, p1 = pearsonr(monthly_df['mod_temp_avg'], monthly_df['efficiency1'])
r2, p2 = pearsonr(monthly_df['mod_temp_avg'], monthly_df['efficiency2'])
r3, p3 = pearsonr(monthly_df['mod_temp_avg'], monthly_df['e_charge'])
...
# 그래프 1: 모듈온도 + efficiency1
axs[0].plot(months, monthly_df['mod_temp_avg'], color='tab:blue', marker='o', label='모듈온도 (°C)')
axs[0].plot(months, monthly_df['efficiency1'], color='tab:green', marker='s', label='주행 효율 e1 (%)')
for x, y1, y2 in zip(months, monthly_df['mod_temp_avg'], monthly_df['efficiency1']):
...
axs[0].set_title('월별 모듈온도 & 주행 효율 e1')
axs[0].set_ylabel('온도 / 효율 (%)')
axs[0].legend()
axs[0].grid(True)

# 그래프 2: 모듈온도 + efficiency2
axs[1].plot(months, monthly_df['mod_temp_avg'], color='tab:blue', marker='o', label='모듈온도 (°C)')
axs[1].plot(months, monthly_df['efficiency2'], color='tab:orange', marker='s', label='주행 효율 e2 (%)')
for x, y1, y2 in zip(months, monthly_df['mod_temp_avg'], monthly_df['efficiency2']):
...
axs[1].text(0.02, 0.95, f'r = {r2:.2f}, p = {p2:.3f}', transform=axs[1].transAxes,
            fontsize=12, color='black', verticalalignment='top')
axs[1].set_title('월별 모듈온도 & 주행 효율 e2')
axs[1].set_ylabel('온도 / 효율 (%)')
axs[1].legend()
axs[1].grid(True)

# 그래프 3: 모듈온도 + e_charge
axs[2].plot(months, monthly_df['mod_temp_avg'], color='tab:blue', marker='o', label='모듈온도 (°C)')
axs[2].plot(months, monthly_df['e_charge'], color='tab:red', marker='s', label='충전 효율 e_charge (%)')
for x, y1, y2 in zip(months, monthly_df['mod_temp_avg'], monthly_df['e_charge']):
...
axs[2].text(0.02, 0.95, f'r = {r3:.2f}, p = {p3:.3f}', transform=axs[2].transAxes,
            fontsize=12, color='black', verticalalignment='top')
axs[2].set_title('월별 모듈온도 & 충전 효율 e_charge')
